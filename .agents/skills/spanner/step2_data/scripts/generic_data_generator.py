...
# --- Curated realistic values for heuristic data synthesis ---
...
def generate_value(prop_name, prop_type, context={}):
    """Heuristically generate realistic mock values based on property name and type."""
    name_lower = prop_name.lower()
    
    # Handle NULL cases if property is nullable
    if context.get("nullable", True) and random.random() < 0.02:
        return None

    # Determine standard business types based on property names
...
def generate_prefix_id(label, index):
    """Generate a clean, readable ID prefix (e.g. cust-0000005)."""
    prefix = label[:4].lower()
    return f"{prefix}-{str(index).zfill(7)}"

# --- Custom Demo Seeding Heuristics ---
def inject_custom_seeding_paths(graph_name, dataset, rows_per_table):
    """Explicitly inject highly connected, predictable paths into the dataset to satisfy presentation queries."""
    import datetime
    ...
    elif graph_name == "AMLGraph":
        print(" [Seeding] Injecting specific paths for AML Laundering Demo...")
        customers = dataset.get("Customers", [])
        accounts = dataset.get("Accounts", [])
        owns = dataset.get("CustomerOwnsAccount", [])
        transfers = dataset.get("AccountTransfers", [])
        pays = dataset.get("AccountPaysMerchant", [])
        merchants = dataset.get("Merchants", [])
        
        if len(accounts) >= 3:
            accounts[0] = {"account_id": "acc-0000000", "bank_name": "Apex Trust", "balance": "150000.00", "currency": "USD"}
            accounts[1] = {"account_id": "acc-0000001", "bank_name": "Nova Bank", "balance": "25000.00", "currency": "USD"}
            accounts[2] = {"account_id": "acc-0000002", "bank_name": "Sovereign Union", "balance": "35000.00", "currency": "USD"}
            
        transfers.insert(0, {"account_id": "acc-0000000", "transfer_id": "tx-0000001", "dest_account_id": "acc-0000001", "amount": "85000.00", "timestamp": now})
        transfers.insert(1, {"account_id": "acc-0000001", "transfer_id": "tx-0000002", "dest_account_id": "acc-0000002", "amount": "84000.00", "timestamp": now})
        transfers.insert(2, {"account_id": "acc-0000002", "transfer_id": "tx-0000003", "dest_account_id": "acc-0000000", "amount": "83500.00", "timestamp": now})
        
        if len(customers) >= 1:
            customers[0] = {"customer_id": "cust-0000000", "name": "Rostov Syndicate", "type": "Corporate Entity", "risk_score": 10, "country": "Russia"}
        if len(merchants) >= 1:
            merchants[0] = {"merchant_id": "merch-0000000", "name": "Monte Carlo Palms Casino", "category": "Gaming/Casino", "risk_level": "HIGH"}
            
        owns.insert(0, {"customer_id": "cust-0000000", "account_id": "acc-0000000", "opened_at": now})
        pays.insert(0, {"account_id": "acc-0000000", "payment_id": "pay-0000001", "merchant_id": "merch-0000000", "amount": "95000.00", "timestamp": now})

def generate_dataset(config, rows_per_table):
    """Dynamically synthesize mock data for nodes and edges based on the use case config."""
    dataset = {}
    node_id_pool = {}  # Map: NodeLabel -> List of Primary Key Dicts (for edge linkage)

    print(f"\n[Data Synthesizer] Generating mock dataset for graph: {config['graph_name']}")
    print(f"[Data Synthesizer] Target scale: {rows_per_table:,} rows per node table.")
    ...
    for node in config.get("nodes", []):
        ...
        print(f" -> Synthesizing node table: {table_name} ({label})...")
        records = []
        node_pks = []
        ...
        for idx in range(rows_per_table):
            ...
            # Generate Primary Key(s)
            # Currently support single key as prefix, composite as random numbers
            for pk in pk_cols:
                if len(pk_cols) == 1:
            ...
            # Store primary keys for linking edges later
            node_pks.append({pk: record[pk] for pk in pk_cols})
            
        dataset[table_name] = records
        node_id_pool[label] = node_pks

    # 2. Generate Edge Tables (Relationships)
    # Scale edges proportionally to node count to keep graph connected
    edge_scale_multiplier = {
        ...
        "Views": 6,
        "Inventory": 2,
        "AccountAssumesRole": 3,
        "RoleHasPermission": 5,
        "ResourceNetworkFlow": 8
    }
    ...
    for edge in config.get("edges", []):
        ...
        # Determine number of edges to generate
        multiplier = edge_scale_multiplier.get(label, 4)
        n_edges = rows_per_table * multiplier
        
        print(f" -> Synthesizing edge table: {table_name} ({label}) — creating {n_edges:,} edges...")
        records = []
        seen_edges = set()

        # Retrieve PK lists for source and destination nodes
        src_pks = node_id_pool.get(src_node, [])
        dst_pks = node_id_pool.get(dst_node, [])

        if not src_pks or not dst_pks:
            print(f"    WARNING: Missing source/destination nodes. Skipping edge generation for {table_name}.")
            continue

        # Force at least one logical "path" for the scenarios to query successfully
        # Path: Node 0 -> Node 1 -> Node 2
        # We can seed some specific connections
        for idx in range(min(10, rows_per_table)):
            record = {}
            context = {"node_label": label, "nullable": False}
            
            # Draw source keys from node index idx
            src_node_pk = src_pks[idx]
            for edge_k, node_k in edge["source"]["key_map"].items():
                record[edge_k] = src_node_pk[node_k]
                
            # Draw destination keys (often index idx or idx+1 to form chains)
            ...
            for pk in pk_cols:
                if pk in record:
                    ...
                    # Discriminator key (like transaction_id or timestamp)
        ...
        while len(records) < n_edges and attempts < max_attempts:
            ...
            # Map source node keys to edge properties
            for edge_k, node_k in edge["source"]["key_map"].items():
                record[edge_k] = src_node_pk[node_k]
                
            # Map destination node keys to edge properties
            for edge_k, node_k in edge["destination"]["key_map"].items():
                record[edge_k] = dst_node_pk[node_k]
                
            # Populate remaining PK columns (like discriminators, transaction_ids)
            edge_pk_vals = []
            for pk in pk_cols:
                if pk in record:
                    edge_pk_vals.append(record[pk])
            ...
            # Avoid duplicate primary keys in the edge dataset
            if edge_tuple not in seen_edges:
                # Generate other non-key properties
                for prop in edge.get("properties", []):
                    if prop["name"] in record:
                        continue
                    context["nullable"] = prop.get("nullable", True)
    ...
    # Inject custom seeding paths to guarantee query success for demos
    inject_custom_seeding_paths(config['graph_name'], dataset, rows_per_table)

    return dataset

def main():
    parser = argparse.ArgumentParser(description="Generic Schema-Driven Spanner Graph Data Synthesizer.")
    parser.add_argument('--config', required=True, help='Path to the use_case_config.json file')
    parser.add_argument('--parameters', default='config/spanner_parameters.json', help='Path to spanner_parameters.json')
    parser.add_argument('--output', required=True, help='Path to output the dummy_data.json')
    args = parser.parse_args()

    # 1. Load Configurations
    if not os.path.exists(args.config):
        print(f"Error: Use-case config file '{args.config}' not found.")
        sys.exit(1)
    
    use_case_config = load_json(args.config)
    
    # Determine rows per table based on active profile in spanner_parameters.json
    rows_per_table = 500  # Default fallback
    if os.path.exists(args.parameters):
        try:
            ...
            print(f"[Data Synthesizer] Loaded active scale profile '{profile_name}' from parameters.")
        except Exception as e:
            print(f"[Data Synthesizer] Warning: Failed to load parameters: {e}. Using default scale.")
    ...
    print(f"\n[Data Synthesizer] Writing generated data to: {args.output}...")
    with open(args.output, 'w') as f:
        json.dump(mock_data, f, indent=2)

    print("\n[Data Synthesizer] Generation completed successfully!")
    for table_name, records in mock_data.items():
        print(f"  - Table '{table_name}': {len(records):,} records generated.")
